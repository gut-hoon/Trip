<template>
  <v-container class="plan-list-container">
    <div class="page-header">
      <h1 class="page-title">📋 내 여행 계획 목록 ✈️</h1>
      <div class="wave-divider"></div>
    </div>

    <v-row class="plans-grid">
      <v-col v-for="trip in myPlan" :key="trip.no" cols="12" lg="4" md="6">
        <v-card
          class="plan-card"
          elevation="8"
          rounded="xl"
          ...
          <v-card-subtitle class="plan-meta">
            <div class="meta-row">
              <span class="meta-item"
                >✏️ {{ memberStore.loginUser.user.name }}</span
              >
...
<script setup>
  ...
  onMounted(async () => {
    const response = await planStore.selectMyPlan(memberStore.loginUser.user.no)
    myPlan.value = response.data // 또는 response.data.data
