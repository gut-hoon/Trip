<template>
  <div class="planner-wrapper">
    <div class="page-header">
      <h1 class="page-title">🗓️ 여행 계획 세우기 ✈️</h1>
      <div class="wave-divider"></div>
      <p class="page-subtitle">나만의 특별한 여행을 계획해보세요!</p>
    </div>
    ...
    <div class="main-layout">
        ...
        <v-card class="plan-card sidebar-card" elevation="8" rounded="xl">
          ...
          <v-card-text class="plan-content">
            ...
            <div v-else class="plan-list">
              <div v-for="(p, i) in plan" :key="p.place_no" class="plan-item">
                <div class="plan-number">{{ i + 1 }}</div>
                <div class="plan-info">
                  <div class="plan-place-name">{{ p.place_name }}</div>
                  <div class="plan-place-address">📍 {{ p.place_address }}</div>
                </div>
                <v-btn
                  icon="mdi-delete"
                  color="red"
      ...
      <div class="main-content">
        ...
        <v-card class="info-card" elevation="6" rounded="xl">
          <v-card-title class="info-title"> ✏️ 여행 정보 입력 </v-card-title>
...
<script setup>
  ...
  // import kakao from 'kakao.maps' // Declare kakao variable
      ...
      ps.keywordSearch(keyword, (data, status, pagination) => {
        if (status === kakao.maps.services.Status.OK) {
          searchResults.value = data.map((item) => ({
            ...
            place_address: item.address_name,
        ...
        const infowindow = new kakao.maps.InfoWindow({
          content: `
            <div style="padding:5px; color:black;">
              ...
              <small style="color:gray;">${place.place_address}</small>
            </div>`,
          removable: true,
        })
  ...
  const addPlaceToPlan = (place) => {
    if (!plan.value.find((p) => p.place_no === place.place_no)) {
      plan.value.push({
        ...place,
  ...
  const saveTripPlan = async () => {
    if (!memberStore.isLoggedIn) {
      ...
      try {
        ...
        await planStore.createPlanDetails(plan.value)
